...
/**
 * 用於攔截所有的 HTTP Request，從 Header 中提取授權資訊，將其轉換為 JWT（JSON Web Token）後，
 * 存儲到 ContextHolder 內以供後續使用。
 * Order 屬性用於設定 Filter 的執行順序，數字越小表示優先級越高，因為目前只有一個 Filter，所以沒有特別設置。
 */
@Component
@Slf4j
